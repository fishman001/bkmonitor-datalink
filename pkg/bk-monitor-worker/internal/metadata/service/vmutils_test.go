...
func TestVmUtils_getBkbaseDataNameAndTopic(t *testing.T) {
	...
	tests := []struct {
		...
		{name: "a-b.c--d.__default__", args: args{tableId: "a-b.c--d.__default__"}, want: "vm_a_b_c_d", want1: fmt.Sprintf("%s%v", "vm_a_b_c_d", viper.GetInt(globalconfig.DefaultBkdataBizIdPath))},
	}
...
func TestVmUtils_AccessBkdata(t *testing.T) {
	...
	funcPatch := gomonkey.ApplyFunc(VmUtils.AccessVmByKafka, func(s VmUtils, tableId, rawDataName, vmClusterName string, timestampLen int) (map[string]interface{}, error) {
		return map[string]interface{}{
			"clean_rt_id":         fmt.Sprintf("%s_%s", viper.GetString(globalconfig.DefaultBkdataBizIdPath), "raw_data_name"),
