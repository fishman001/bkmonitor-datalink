...
var KafkaTopicPrefixStorage = fmt.Sprintf("0%s_storage_", viper.GetString(api.BkApiAppCodePath))
...
func (a KafkaStorageSvc) CreateTable(tableId string, isSyncDb bool, storageConfig *optionx.Options) error {
	...
	if !ok {
		if id := viper.GetUint(globalconfig.DefaultKafkaStorageClusterIdPath); id != 0 {
			storageClusterId = id
		} else {
